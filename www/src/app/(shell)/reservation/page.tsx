...
import { Ticket, Check, MapPin, Users, Mic } from "lucide-react";
import { createReservation, findNearbyBranches, listReservations, cancelReservation, type Reservation, type Branch } from "@/lib/api";
...
export default function ReservationPage() {
  ...
  useEffect(() => {
    ...
    listReservations().then(setTickets).catch(() => setTickets([]));
  }, []);
  ...
  async function reserve() {
    ...
    setLoading(true);
    try {
      setResult(await createReservation(purpose, selectedBranch.name, note || undefined));
      listReservations().then(setTickets).catch(() => {});
    } catch {
      setResult(null);
    } finally {
      setLoading(false);
                    ...
                    >
                      <span
                        className={`flex size-12 shrink-0 items-center justify-center rounded-xl text-xl font-black text-white ${
                          canceled ? "bg-slate-300" : "bg-gradient-to-b from-jb-400 to-jb-700"
                        }`}
                      >
                        {t.ticket_number}
                      </span>
