...
export default function ChatPage() {
  ...
  const [handoff, setHandoff] = useState<StaffHandoffCard | null>(null);
  ...
  async function requestHandoff() {
    ...
    setLoading(true);
    try {
      const { card, confirm_message } = await handoffToStaff(messages);
      setHandoff(card);
  ...
  return (
    <div className="relative flex h-dvh flex-col overflow-hidden bg-gradient-to-b from-[#0b1026] via-[#141a3a] to-[#241a44] text-white">
      ...
      <div ref={scrollRef} className="relative flex-1 space-y-4 overflow-y-auto px-5 py-5">
        ...
        {handoff && (
          <div>
            <p className="mb-2 text-base font-bold text-jb-100">📋 직원에게 전달된 내용</p>
            <HandoffCard card={handoff} />
          </div>
        )}
      </div>
