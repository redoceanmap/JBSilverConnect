...
class FallbackPhishingLlmAdapter(PhishingLlmPort):
    """
    데코레이터 패턴 + LSP.
    앞선 어댑터가 실패하면 다음 순위로 폴백한다. 호출부는 단일 PhishingLlmPort만 알면 된다.
    분기(if-else)가 아니라 순차 시도(loop + 예외 처리)로 OCP·LSP를 만족.
    """

    def __init__(self, adapters: list[PhishingLlmPort]) -> None:
        if not adapters:
